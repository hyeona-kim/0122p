...
<body>
	<article id="wrap">
			...
			<div class="right">
				<!--  여기서 표시될 테이블들 가지고오기 -->
					...
					<div id="counselWrap">
						...
						<div id="ttop">
							<button type="button" onclick="set()">상단파일등록</button>	
							<button type="button" onclick="set2()">상담파일등록 샘플</button>
						</div>
						<form>
		...
		<div id="dialog2" hidden="" title="상담파일등록 샘플">
		</div>
	...
	<script>
		...
		function set() {
			$.ajax({
				url:"Controller",
				type:"post",
				data:"type="+encodeURIComponent("c_dialog")+"&select="+encodeURIComponent("addCounsel")
			}).done(function(result){
				$("#dialog").html(result);
